...
private:
   ...
   bool insertHelp(TreeNode* ptr, DBentry* newEntry);
   
   // used to find a node with the same name as the one given
   DBentry* findHelp(TreeNode* ptr, string name);
   
   
   // used to delete the entire BST
   void clearHelp(TreeNode* ptr);
   
   // used to count all of the active nodes in the list
   int countActiveHelp(TreeNode* ptr) const;
   
   // used to print the list in order from left to right
   void printHelp(TreeNode* ptr) const;
   
   bool removeHelp(TreeNode* parent, string name, TreeNode* prev);
   
   TreeNode* maxNode(TreeNode* parent,TreeNode*& prevMax);
